(() => {
  ...
  // ---------- ztconsole: 生成中のライブ演出（実体は assets/zt-console.js。ここは入力値を渡すだけ） ----------
  // 見本プレビュー(#sample-site)に表示中の文言をそのまま渡し、演出の「AIが書いている文章」と右の骨組みに使う。
  const ztInput = () => {
    const visibleTypeBlock = Object.values(typeBlocks).find((block) => block && !block.hidden);
    const labelOf = (block, selector) => (block && block.querySelector(selector) ? block.querySelector(selector).textContent : "");
    const menu = Array.from(menuPreviewList.querySelectorAll(".sample-menu-item")).map((item) => ({
      name: labelOf(item, ".sample-menu-item-name"),
      price: labelOf(item, ".sample-menu-item-price")
    }));
    const previewText = (key) => (previewValues[key] ? previewValues[key].textContent : "");
    return {
      shopName: previewText("shopName"),
      businessType: previewText("businessType"),
      catchcopy: previewText("catchcopy"),
      description: previewText("description"),
      featureLabel: labelOf(visibleTypeBlock, ".sample-block-label"),
      featureTitle: labelOf(visibleTypeBlock, "strong"),
      menu,
  ...
  const ztStart = () => {
    if (window.ztConsole) window.ztConsole.start(ztInput());
  };
  const ztStop = () => {
    if (window.ztConsole) window.ztConsole.stop();
  };
  ...
  const submitGeneration = async () => {
    ...
    generationStatus.textContent = "AIが、あなたのホームページを組み立てています";
    setGenerationState("loading");
    ztStart();
    // 生成が速すぎても演出が一瞬で消えないよう、最低表示時間を確保する（体感の「作っている感」のため）
    const ZT_MIN_VISIBLE_MS = 6000; // 5工程の切替演出（READING→DEPLOYING）まで見せるため。実生成は通常これより長い
